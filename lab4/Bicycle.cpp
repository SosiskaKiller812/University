...
double Bicycle::getTime(double distance, int persons, double extraWeight) const{
	...
	int kWeight = sqrt(persons * avgPeopleWeight / (persons * avgPeopleWeight + extraWeight));
	return distance / (kmPh * kWeight);
}
...
double Bicycle::getPrice(double distance, int persons, double extraWeight) const{
	...
	return pricePerHuman * persons + extraWeight * bicyclePricePk;
}
